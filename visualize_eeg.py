...
def main():
    ...
    parser.add_argument('--live', action='store_true', help='Run live-updating plot (tails CSV)')
    parser.add_argument('--interval', type=float, default=1.0, help='Polling interval for live mode (seconds)')
    parser.add_argument('--window', type=float, default=10.0, help='Time window (seconds) for live plot')
    parser.add_argument('--band', type=str, default=None, help='Band to plot (delta, theta, alpha, beta, gamma)')
    parser.add_argument('--sf', type=float, default=256.0, help='Sampling frequency (Hz) of the EEG data')

    args = parser.parse_args()

    if args.live:
        plot_live(poll_interval=args.interval, window=args.window, band=args.band, sf=args.sf)
    else:
