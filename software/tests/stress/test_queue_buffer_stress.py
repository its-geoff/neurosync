...
class TestQueueBufferStress:
    ...
    def test_slow_consumer_does_not_deadlock_producer(self):
        ...
        with mock.patch("graphing.time.sleep"):
            producer = threading.Thread(target=graphing.write_data, args=(fft_df, buf))
        ...
        assert not producer.is_alive(), "producer thread still running (deadlock?)"
    ...
    def test_final_frame_always_present_after_write(self):
        for n in [10, 50, 100, 200]:
            ...
            assert len(frame) == n, f"n={n}: expected {n} rows, got {len(frame)}"
