...
def _fake_samples(n: int) -> list:
    """Generate n fake EEG samples, each with 5 channels (main.py slices [:4])."""
...
def _patch_inlet(samples):
    """Patch main.StreamInlet to return a _SampleInlet for the given samples."""
    return patch("main.StreamInlet", side_effect=lambda _: _SampleInlet(samples))
...
def _patch_fft(return_value=None):
    rv = pd.DataFrame([{"delta": 1, "theta": 2, "alpha": 3, "beta": 4}]) if return_value is None else return_value
...
class TestConnectAndProcessBuffering(unittest.TestCase):
    ...
    def test_no_processing_before_256_samples(self):
        """If fewer than 256 samples arrive, transform_to_hz is never called."""
        with _patch_resolve(), \
    ...
    def test_processing_triggers_twice_for_384_samples(self):
        """
        384 samples = first window at 256, then 128 remaining + 128 new = second window.
...
class TestConnectAndProcessWindowShape(unittest.TestCase):
    ...
    def test_fft_called_with_dataframe(self):
        ...
        def capture_fft(df):
            captured["arg"] = df
            return pd.DataFrame([{"delta": 1, "theta": 2, "alpha": 3, "beta": 4}])
    ...
    def test_fft_receives_256_row_window(self):
        """The DataFrame passed to transform_to_hz must have exactly 256 rows."""
        captured = {}
    ...
    def test_samples_sliced_to_four_channels(self):
        """main.py does sample[:4] — fifth channel must not reach the window."""
        captured = {}
...
class TestConnectAndProcessOverlap(unittest.TestCase):
    """Tests the 50% window overlap — buffer[:128] is kept after each window."""

    def test_second_window_uses_overlap_from_first(self):
        """
        After the first window (samples 0–255), the buffer keeps samples 128–255.
    ...
    def test_buffer_not_fully_cleared_between_windows(self):
        ...
        would fire for 383 samples. Two windows means overlap is working.
        """
        call_count = {"n": 0}
...
class TestConnectAndProcessTransmission(unittest.TestCase):
    ...
    def test_ser_write_called_for_each_fft_row(self):
        """ser.write() must be called once per row returned by transform_to_hz."""
    ...
    def test_df_to_packet_called_for_each_row(self):
        ...
        fft_output = pd.DataFrame([{"delta": 10, "theta": 20, "alpha": 30, "beta": 40}])

        with _patch_resolve(), \
             _patch_inlet(_fake_samples(256)), \
             _patch_fft(return_value=fft_output), \
             patch("main.transmission.df_to_packet",
                   return_value=b"\x00" * 12) as mock_packet:
            main.connect_and_process(_make_fake_ser())

        mock_packet.assert_called_once()

    def test_packet_written_to_serial_is_from_df_to_packet(self):
        """The bytes passed to ser.write() must come directly from df_to_packet."""
...
class TestConnectAndProcessShutdown(unittest.TestCase):
    ...
    def test_keyboard_interrupt_stops_loop(self):
        """Loop must exit cleanly on KeyboardInterrupt — no exception propagates."""
        with _patch_resolve(), _patch_inlet([]):
            try:
                main.connect_and_process(_make_fake_ser())
            except KeyboardInterrupt:
                self.fail("KeyboardInterrupt was not caught inside connect_and_process")

    def test_partial_buffer_discarded_on_interrupt(self):
        """Samples in a partial buffer (< 256) are discarded without processing."""
        with _patch_resolve(), \
